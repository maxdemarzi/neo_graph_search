...
describe "NGS::Parser" do
  ...
  describe "basics" do
      it "can return me" do
        cypher = "START me = node({me}) RETURN me"
        params = {"me" => nil}
        NGS::Parser.parse("(me)").should == [cypher, params]
      end

      it "can return my friends" do
        cypher = "START me = node({me}) MATCH me -[:friends]-> friends RETURN friends"
        params = {"me" => nil}
        NGS::Parser.parse("(friends)").should == [cypher, params]
      end

      it "can return friends who like cheese" do
        cypher = "START me = node({me}), thing = node:things(name:{thing}) MATCH me -[:friends]-> friends, friends -[:likes]-> thing RETURN friends"
        params = {"me" => nil, "thing" => "cheese"}
        NGS::Parser.parse("(friends who like cheese)").should == [cypher, params]
      end

      it "can return friends who like cheese and crackers" do
        cypher = "START me = node({me}), thing1 = node:things(name:{thing1}), thing2 = node:things(name:{thing2}) MATCH me -[:friends]-> friends, friends -[:likes]-> thing1, friends -[:likes]-> thing2 RETURN friends"
        params = {"me" => nil, "thing1" => "cheese", "thing2" => "crackers"}
